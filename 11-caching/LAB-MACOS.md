# Lab 11 -- macOS/Linux Instructions
...
```bash
cd 11-caching
chmod +x setup.sh cleanup.sh scripts/seed-database.sh
./setup.sh
```
...
```bash
time curl -s http://localhost:5050/products/1 | python3 -m json.tool
```
...
In a second terminal, start monitoring Redis commands in real time:
...
```bash
docker exec redis-cache redis-cli INFO stats | grep evicted_keys
```
...
```bash
docker ps -a | grep -E "redis-cache|backend-db|cache-lab"
```
